...
def analyze_sweep(sweep_id):
    ...
    # Nested dictionary: lambda -> lr -> list of runs/metrics
    ...
    for run in runs:
        if run.state == "finished":
            ...
            if kf_mse is not None and metrics:
                ...
                grouped_runs[l_val][lr].append(row)
    ...
    # Header
    header = f"{'lambda':<8} | {'Best LR':<10} | {'KF MSE':<10}"
    ...
    for l_val in sorted(grouped_runs.keys()):
        best_lr = None
        best_score = float('inf')
        best_rows = None
        
        # Find best LR based on t+2 score (most sensitive metric)
        for lr, rows in grouped_runs[l_val].items():
            # Filter rows that actually have t+2
        ...
        if best_rows:
            ...
            line = f"{l_val:<8.2f} | {best_lr:<10.4f} | {avg_kf:<10.3e}"
...
if __name__ == "__main__":
    analyze_sweep("nzucchet/Transformer-filters/i724zgfm")