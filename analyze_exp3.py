...
def analyze_sweep(sweep_id):
    ...
    # Nested dictionary: (lambda, diag, patch) -> lr -> list of runs/metrics
    ...
    for run in runs:
        if run.state == "finished":
            ...
            patch = run.config.get("patch_size")
            ...
            if kf_mse is not None and metrics:
                ...
                grouped_runs[(l_val, diag, patch)][lr].append(row)
    ...
    # Header
    header = f"{'lambda':<8} | {'Diag':<5} | {'Patch':<5} | {'Best LR':<10} | {'KF MSE':<10}"
    ...
    for (l_val, diag, patch) in sorted(grouped_runs.keys()):
        best_lr = None
        best_score = float('inf')
        best_rows = None
        
        # Find best LR based on t+2 score
        for lr, rows in grouped_runs[(l_val, diag, patch)].items():
        ...
        if best_rows:
            ...
            line = f"{l_val:<8.2f} | {diag:<5} | {patch:<5} | {best_lr:<10.4f} | {avg_kf:<10.3e}"
