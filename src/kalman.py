...
def kalman_filter(y, A, C, Q_std, R_std=1e-4, b=None, P0=None):
    """
    Runs Kalman Filter.
    y: (T, dy)
    A: (dx, dx)
    C: (dy, dx)
    Q_std: scalar
    b: optional (dx,) affine bias for state transition
    P0: optional (dx, dx) initial covariance (defaults to Q)
    """
    T, dy = y.shape
    dx = A.shape[0]
    dtype = A.dtype
    ...
    P_init = P0 if P0 is not None else Q
    bias = b if b is not None else jnp.zeros(dx, dtype=dtype)
...
def kalman_filter_final_state(y, A, C, Q_std, R_std=1e-4, b=None, P0=None):
    """
    Returns the final posterior state x_{T|T}.
    y: (T, dy)
    b: optional (dx,) affine bias
    P0: optional (dx, dx) initial covariance (defaults to Q)
    """
    T, dy = y.shape
    dx = A.shape[0]
