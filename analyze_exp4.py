...
def analyze_sweep(sweep_id):
    ...
    # Nested dictionary: (dim_y, patch) -> lr -> list of runs/metrics
    ...
    for run in runs:
        if run.state == "finished":
            ...
            if kf_mse is not None and metrics:
                ...
                grouped_runs[(dy, patch)][lr].append(row)
    ...
    # Header
    header = f"{'dim_y':<5} | {'Patch':<5} | {'Best LR':<10} | {'KF MSE':<10}"
    ...
    for (dy, patch) in sorted(grouped_runs.keys()):
        best_lr = None
        best_score = float('inf')
        best_rows = None
        
        # Find best LR based on t+2 score
        for lr, rows in grouped_runs[(dy, patch)].items():
        ...
        if best_rows:
            ...
            line = f"{dy:<5} | {patch:<5} | {best_lr:<10.4f} | {avg_kf:<10.3e}"
